...
const test = base.extend<WorkerFixtures>({
  backend: [
    async (
      ...
      const workDir = "/tmp/cydo-sandbox-env-djinja";
      ...
      writeTestConfig(
        ...
        [
          ...
          "    sandbox:",
          "      env:",
          '        CYDO_TEST_TITLE: "secret-{{ env.CYDO_TEST_SECRET_SOURCE }}"',
          "workspaces:",
          "  local:",
...
test("agents[*].sandbox.env values render Djinja env templates", async ({
  backend,
}) => {
  ...
  ws.send(
    JSON.stringify({
      ...
      entry_point: "agentic",
      correlation_id: "sandbox-env-djinja",
    }),
  );
